...
stratiolaelaps_table<-read.table(file.path(script_path, "data.txt"), header=TRUE)
colnames(stratiolaelaps_table)[which(names(stratiolaelaps_table) == "days")] <- "age"
colnames(stratiolaelaps_table)[which(names(stratiolaelaps_table) == "mm")] <- "length"

stratiolaelaps_data <- split(stratiolaelaps_table, stratiolaelaps_table$temperature)
...
female_predicted_length <- predict(best_model, 
	newdata = data.frame(age = max(stratiolaelaps_data[[as.character(tempFromLab)]]))
)
...
json.femaleMaxReproductionEvents = 30  # Maximum number of reproduction events for a female
...
json.traits.base.actE_met.limits.max = 1.385496   # Upper limit on individual variation
json.traits.base.actE_met.limits.min = 0.4272623  # Lower limit on individual variation
json.traits.base.actE_met.ranges.max = 0.4192  # Upper range boundary for sampling initial trait values
json.traits.base.actE_met.ranges.min = 0.3492  # Lower range boundary for sampling initial trait values
...
json.traits.base.devTime.ranges.max = 11.0  # If max and min = max(ageVector), no genetic variation
json.traits.base.devTime.ranges.min = 11.0  # If max and min = max(ageVector), no genetic variation
...
## base - growth
json.traits.base.growth.limits.max = 2.526715   # Maximum growth rate limit (0 means no constraint or zero?)
json.traits.base.growth.limits.min = 0.9980861  # Minimum growth rate limit
json.traits.base.growth.ranges.max = 0.05671  # Upper boundary for sampling growth rate
json.traits.base.growth.ranges.min = 0.05225  # Lower boundary for sampling growth rate
...
json.traits.base.coeffMassForSearchRadius.limits.max = 0.75
json.traits.base.coeffMassForSearchRadius.limits.min = 0.9
json.traits.base.coeffMassForSearchRadius.ranges.max = 60.0 
json.traits.base.coeffMassForSearchRadius.ranges.min = 50.0
...
json.traits.base.coeffMassForSpeed.limits.max = 0.4545455 
json.traits.base.coeffMassForSpeed.limits.min = 0.8333333 
json.traits.base.coeffMassForSpeed.ranges.max = 5.5 
json.traits.base.coeffMassForSpeed.ranges.min = 3.0
...
json.traits.base.longevitySinceMaturation.ranges.max = 4.0 
json.traits.base.longevitySinceMaturation.ranges.min = 4.0
...
## base - met_rate
json.traits.base.met_rate.limits.max = 0.213419
json.traits.base.met_rate.limits.min = 0.7060985
...
json.growthModule.ageVector = c(1.0, 6.0, 11.0) # Taken from Section 1, already parametrised
...
json.growthModule.eggDryMass.value = 0.00311  # Default fixed egg dry mass if not from equation
...
json.growthModule.eggsPerBatch.value = 2  # Fixed number of eggs per batch
...
json.growthModule.instarFirstReproduction = 4
...
json.name = "Stratiolaelaps_sp"  # Name of the species or simulation run
