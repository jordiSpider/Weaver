...
source(file.path(parametrisation_path, "utilities", "codes", "functions", "calculateIndividualsPerInstarTanypod.r"))
...
# Pupal exuvia table given by Encarni to re-scale lengths based on 
# data from other chironomid 

families<-c("Crironomidae","Tanypodinae")
min<-c(3.8,8.2)
max<-c(6.6, 10.3)
mean<-c(5.2, 9.25)
table<-data.frame(families,min,max,mean)

scale_factor<-max(table$mean)/min(table$mean)
scale_factor #@scale_factor=1.778846

# So now we can read the table of the close species and re-scale

cricotopus_table<-read.table(file.path(script_path, "Chironomidae_Cricotopus.txt"),header=T)
...
cricotopus_table<-subset(cricotopus_table, temperature ==10)
tanypod_length<-cricotopus_table$length*scale_factor

tanypod_table<-data.frame(age=cricotopus_table$age,
                          length=tanypod_length)



# Start reading ID_TANHYP_tempFromLab data frame
ID_TANHYP_tempFromLab<-data.frame(
			Variable = "tempFromLab",
			DOI1 = NA,
			Category1 = NA,
			DataTreatment = NA
)
# End of ID_TANHYP_tempFromLab data frame
tempFromLab<-10
tempFromLab #@tempFromLab=10


# Start reading ID_TANHYP_sexRatio data frame		  
ID_TANHYP_sexRatio<-data.frame(
			Variable = "sexRatio", 
			DOI1 = NA,
			Category1 = NA,
			DataTreatment = NA
)
# End of ID_TANHYP_sexRatio data frame

sexRatio<-0.5
sexRatio #@sexRatio=0.5

# Start reading ID_TANHYP_conversionToWetMass data frame
ID_TANHYP_conversionToWetMass<-data.frame(
...
# End of ID_TANHYP_conversionToWetMass data frame

conversionToWetMass<-9.193
conversionToWetMass #@conversionToWetMass=9.193

# Start reading ID_TANHYP_instarFirstReproduction data frame
ID_TANHYP_instarFirstReproduction<-data.frame(
...
# End of ID_TANHYP_instarFirstReproduction data frame

instarFirstReproduction<-5
instarFirstReproduction #@instarFirstReproduction=5


# Start reading ID_TANHYP_eggsPerBatch data frame
ID_TANHYP_eggsPerBatch<-data.frame(
			Variable = "eggsPerBatch",
			DOI1 = NA,
			Category1 = NA,
			DataTreatment = "extracted from json in the folder"
)
# End of ID_TANHYP_eggsPerBatch data frame

eggsPerBatch<-round(250*scale_factor,0)
eggsPerBatch #@eggsPerBatch=20

# Start reading ID_TANHYP_femaleMaxReproductionEvents data frame
ID_TANHYP_femaleMaxReproductionEvents<-data.frame(
			Variable = "femaleMaxReproductionEvents", 
			DOI1 = NA,
			Category1 = NA, 
			DataTreatment = "extracted from json in the folder"
)
# End of ID_TANHYP_femaleMaxReproductionEvents data frame

femaleMaxReproductionEvents<-2
femaleMaxReproductionEvents #@femaleMaxReproductionEvents=2 #@femaleMaxReproductionEvents=5

# Start reading ID_TANHYP_fieldDensity data frame
ID_TANHYP_fieldDensity<-data.frame(
			Variable = "fieldDensity", 
			DOI1 = NA,
			Category1 = NA, 
			DataTreatment = "extracted from json in the folder"
)
# End of ID_TANHYP_fieldDensity data frame

fieldDensity=223  ##inds/sq_meter
fieldDensity=fieldDensity/1e+6 ##inds/sq_mm

# Start reading ID_TANHYP_ageVector data frame
ID_TANHYP_ageVector<-data.frame(
			Variable = "ageVector",
			DOI1 = NA ,
			Category1 = NA ,
			DataTreatment = NA 
)		
# End of ID_TANHYP_ageVector data frame


ageVector<-c(7.47,13.39,20.14,29.04)
ageVector #@ageVector=c(7.47,13.39,20.14,29.04)

# Start reading ID_TANHYP_coefficientForMassA data frame
ID_TANHYP_coefficientForMassA<-data.frame(
...
# End of ID_TANHYP_coefficientForMassA data frame

coefficientForMassA<-0.0038
coefficientForMassA #@coefficientForMassA=0.002


# Start reading ID_TANHYP_scaleForMassB data frame
ID_TANHYP_scaleForMassB<-data.frame(
			Variable = "scaleForMassB", 
			DOI1 = NA,
			Category1 = NA,
			DataTreatment = "Extracted from the script testGrowthAndReproduction_JML_MCS_fixing_and_testing_temperature_vs2"
)
# End of ID_TANHYP_scaleForMassB data frame

scaleForMassB<-2.411
scaleForMassB #@scaleForMassB=2.254


# Start reading ID_TANHYP_coefficientForMassAforMature data frame
ID_TANHYP_coefficientForMassAforMature<-data.frame(
			Variable = "coefficientForMassAforMature", 
			DOI1 = NA,
			Category1 = NA,
			DataTreatment = "Extracted from the script testGrowthAndReproduction_JML_MCS_fixing_and_testing_temperature_vs2"
)
# End of ID_TANHYP_coefficientForMassAforMature data frame

coefficientForMassAforMature<-coefficientForMassA
coefficientForMassAforMature #@coefficientForMassAforMature=0.002


# Start reading ID_TANHYP_scaleForMassBforMature data frame
ID_TANHYP_scaleForMassBforMature<-data.frame(
			Variable = "scaleForMassBforMature", 
			DOI1 = NA,
			Category1 = NA,
			DataTreatment = "Extracted from the script testGrowthAndReproduction_JML_MCS_fixing_and_testing_temperature_vs2"
)
# End of ID_TANHYP_scaleForMassBforMature data frame
...
# Start reading ID_TANHYP_curveType data frame
ID_TANHYP_curveType<-data.frame(
			Variable = "curveType",
			DOI1 = NA,
			Category1 = NA, 
			DataTreatment = NA
)			
# End of ID_TANHYP_curveType data frame


tanypod_hypothetical_data <- list()

tanypod_hypothetical_data[[as.character(tempFromLab)]] = tanypod_table


fitted<-gcmGAT(tanypod_hypothetical_data, x = "age", y = "length", use_app = TRUE)


fitted[[as.character(tempFromLab)]]$adjusted_models_table

#   Temperature       Model      AIC     AICc       RSS best_candidate    Loglik df p_value_LRT
# 1          NA Exponential 67.59451 68.17422  9.694117           TRUE -29.79725  4        <NA>
# 2          NA    Logistic 98.40563 98.98534 14.700561          FALSE -45.20281  4    2.22E-16
# 3          NA      Linear 71.03623 71.37909 10.433857          FALSE -32.51812  3    1.97E-02

curveType<-fitted[[as.character(tempFromLab)]]$best_model_name
curveType #@curveType="Exponential"


# Start reading ID_TANHYP_lengthAtBirth data frame
ID_TANHYP_lengthAtBirth<-data.frame(
			Variable = "lengthAtBirth", 
			DOI1 = NA,
			Category1 = NA, 
			DataTreatment = NA 
)
# End of ID_TANHYP_lengthAtBirth data frame
best_model<-fitted[[as.character(tempFromLab)]]$best_model
lengthAtBirth<-predict(best_model, newdata = data.frame(age = 0))
lengthAtBirth #@lengthAtBirth=0.7429089

# Start reading ID_TANHYP_eggDryMass data frame
ID_TANHYP_eggDryMass<-data.frame(
			Variable = NA,
			DOI1 = NA,
			Category1 = NA,
			DataTreatment = NA
)
# End of ID_TANHYP_eggDryMass data frame

eggDryMass<-coefficientForMassA*lengthAtBirth^scaleForMassB
eggDryMass #@eggDryMass= 0.001856133

# Start reading ID_TANHYP_femaleWetMass data frame
ID_TANHYP_femaleWetMass<-data.frame(
			Variable = "femaleWetMass",
			DOI1 = NA,
			Category1 = NA,
			DataTreatment = NA
)
# End of ID_TANHYP_femaleWetMass data frame

female_predicted_length <- predict(best_model, newdata = 
                           data.frame(age = max(tanypod_table$age)))
femaledrymass<-coefficientForMassA*female_predicted_length^scaleForMassB
femaleWetMass<-femaledrymass*conversionToWetMass
femaleWetMass  #@femaleWetMass=13.10111


# Start reading ID_TANHYP_lMin data frame
ID_TANHYP_lMin<-data.frame(
			Variable = "lMin",
			DOI1 = NA,
			Category1 = NA,
			DataTreatment = NA
)
# End of ID_TANHYP_lMin data frame

lMin<-female_predicted_length
lMin #@lMin=6.568956

# Start reading ID_TANHYP_lMax data frame
ID_TANHYP_lMax<-data.frame(
			Variable = "lMax",
			DOI1 = NA,
			Category1 = NA, 
			DataTreatment = NA
)
# End of ID_TANHYP_lMax data frame
...
# Start reading ID_TANHYP_growthK data frame
ID_TANHYP_growthK<-data.frame(
			Variable = "growth",
			DOI1 = NA,
			Category1 = NA,
			DataTreatment = NA
)
# End of ID_TANHYP_growthK data frame


growthK<-fitted[[as.character(tempFromLab)]]$growth
growthK #@growthK= 0.00881962

# Start reading ID_TANHYP_pupaPeriodTime  data frame
ID_TANHYP_pupaPeriodTime <-data.frame(
			Variable = "pupaPeriodTime",
			DOI1 = NA,
			Category1 = NA, 
			DataTreatment = NA
)
# End of ID_TANHYP_pupaPeriodTime  data frame

pupaPeriodTime <- 1
pupaPeriodTime  #@pupaPeriodTime =1


# Start reading ID_TANHYP_longevitySinceMaturation data frame
ID_TANHYP_longevitySinceMaturation<-data.frame(
			Variable = "longevitySinceMaturation",
			DOI1 = NA,
			Category1 = NA,
			DataTreatment = "Taken from the JSON in the folder"
)
# End of ID_TANHYP_longevitySinceMaturation data frame

longevitySinceMaturation<-4.25

# Start reading ID_TANHYP_assiM data frame	
ID_TANHYP_assiM<-data.frame(
			Variable = "assiM", 
			DOI1 = "",
			Category1 = "Article",
			DataTreatment = ""
)
# End reading ID_TANHYP_assiM data frame
...
# Start reading ID_TANHYP_growthKmin data frame
ID_TANHYP_growthKmin<-data.frame(
			Variable = "growthKmin",
			DOI1 =  NA,
			Category1 = NA,
			DataTreatment = NA
)
# End of ID_TANHYP_growthKmin data frame

growthKmin<-growthK

# Start reading ID_TANHYP_growthKmax data frame
ID_TANHYP_growthKmax<-data.frame(
			Variable = "growthKmax",
			DOI1 = NA,
			Category1 = NA,
			DataTreatment = NA
)
# End of ID_TANHYP_growthKmax data frame
...
# I decided to input 0.80 due to our knowledge of this value for predators in other species
...
json.individualsPerInstar = calculateIndividualsPerInstarTanypod(
	totalNumberOfIndividuals = 223,
...
json.name = "Tanypod_hypothetical"  # Name of the species or simulation run
