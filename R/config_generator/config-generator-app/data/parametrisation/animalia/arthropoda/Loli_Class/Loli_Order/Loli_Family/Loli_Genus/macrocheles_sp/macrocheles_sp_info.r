...
macrocheles_table<-read.table(file.path(script_path, "data.txt"), header=TRUE)
colnames(macrocheles_table)[which(names(macrocheles_table) == "days")] <- "age"
colnames(macrocheles_table)[which(names(macrocheles_table) == "mm")] <- "length"

macrocheles_data <- split(macrocheles_table, macrocheles_table$temperature)
...
female_predicted_length <- predict(best_model, 
	newdata = data.frame(age = max(macrocheles_data[[as.character(tempFromLab)]]))
)
...
json.femaleMaxReproductionEvents = 60  # Maximum number of reproduction events for a female
...
json.traits.base.assim.limits.min = 0.2       # Lower limit on assimilation rate
...
json.traits.base.devTime.ranges.max = 5.0  # If max and min = max(ageVector), no genetic variation
json.traits.base.devTime.ranges.min = 5.0  # If max and min = max(ageVector), no genetic variation
...
## base - growth
json.traits.base.growth.limits.max = 0.8471485  # Maximum growth rate limit (0 means no constraint or zero?)
json.traits.base.growth.limits.min = 0.9994892  # Minimum growth rate limit
json.traits.base.growth.ranges.max = 0.21655  # Upper boundary for sampling growth rate
json.traits.base.growth.ranges.min = 0.19579  # Lower boundary for sampling growth rate
...
json.traits.base.coeffMassForSpeed.limits.max = 0.8181818 
json.traits.base.coeffMassForSpeed.limits.min = 0.8076923
json.traits.base.coeffMassForSpeed.ranges.max = 4.4 
json.traits.base.coeffMassForSpeed.ranges.min = 2.6
...
json.traits.base.longevitySinceMaturation.ranges.max = 13.64 
json.traits.base.longevitySinceMaturation.ranges.min = 13.64
...
json.growthModule.ageVector = c(1.0, 3.0, 5.0) # Taken from Section 1, already parametrised
...
json.growthModule.eggDryMass.value = 0.00478  # Default fixed egg dry mass if not from equation
...
json.growthModule.eggsPerBatch.value = 1  # Fixed number of eggs per batch
...
json.name = "Macrocheles_sp"  # Name of the species or simulation run
