...
eupodidae_table<-read.table(file.path(script_path, "data.txt"), header=TRUE)
colnames(eupodidae_table)[which(names(eupodidae_table) == "days")] <- "age"
colnames(eupodidae_table)[which(names(eupodidae_table) == "mm")] <- "length"

eupodidae_data <- split(eupodidae_table, eupodidae_table$temperature)
...
female_predicted_length <- predict(best_model, 
	newdata = data.frame(age = max(eupodidae_data[[as.character(tempFromLab)]]))
)
...
json.femaleMaxReproductionEvents = 45  # Maximum number of reproduction events for a female
...
json.traits.base.actE_met.limits.min = 0.4771242  # Lower limit on individual variation
json.traits.base.actE_met.ranges.max = 0.4325  # Upper range boundary for sampling initial trait values
json.traits.base.actE_met.ranges.min = 0.3825  # Lower range boundary for sampling initial trait values
...
json.traits.base.devTime.ranges.max = 19.0  # If max and min = max(ageVector), no genetic variation
json.traits.base.devTime.ranges.min = 19.0  # If max and min = max(ageVector), no genetic variation
...
## base - growth
json.traits.base.growth.limits.max = 23.5821    # Maximum growth rate limit (0 means no constraint or zero?)
json.traits.base.growth.limits.min = 0.9908759  # Minimum growth rate limit
json.traits.base.growth.ranges.max = 0.02034  # Upper boundary for sampling growth rate
json.traits.base.growth.ranges.min = 0.01096  # Lower boundary for sampling growth rate
...
json.traits.base.coeffMassForSearchRadius.limits.max = 0.4736842
json.traits.base.coeffMassForSearchRadius.limits.min = 0.5294118 
json.traits.base.coeffMassForSearchRadius.ranges.max = 95.0 
json.traits.base.coeffMassForSearchRadius.ranges.min = 85.0
...
json.traits.base.coeffMassForSpeed.limits.max = 0.5384615 
json.traits.base.coeffMassForSpeed.limits.min = 0.975 
json.traits.base.coeffMassForSpeed.ranges.max = 26.0 
json.traits.base.coeffMassForSpeed.ranges.min = 20.0
...
json.growthModule.ageVector = c(5.0, 10.0, 15.0, 19.0) # Taken from Section 1, already parametrised
...
json.growthModule.eggDryMass.value = 0.00025  # Default fixed egg dry mass if not from equation
...
json.name = "Eupodidae_sp"  # Name of the species or simulation run
