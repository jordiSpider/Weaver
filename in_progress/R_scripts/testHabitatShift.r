...
show_resource_species_info <- function(fig, species_name, info, species_color) {
    ...
    cells_split <- split(info[timeStep == 1], by = c("X", "Y"))
...
show_animal_species_info <- function(fig, species_name, info, species_color, grid_size) {
    fig <- fig %>% 
        add_trace(
            ...
            legendgroup = species_name,  # Agrupar en la leyenda
            showlegend = TRUE,
            text = ~paste("TimeStep:", timeStep, "<br>Species:", species_name, "<br>Id:", id, "<br>X:", X, "<br>Y:", Y, "<br>Instar:", instar, "<br>Life stage:", state, "<br>Repro counter:", reproCounter),  # Texto de la etiqueta
            hoverinfo = 'text'
        )
...
# config_path <- file.path(script_path, "example_output")
config_path <- "/home/macarse/Escritorio/EEZA/Weaver/output/encarni_config__2025-3-11_12-38-47"

resource_species <- c("Biofilm", "Plant1", "Plant2")

animal_species <- c("Cricotopus_spx")
