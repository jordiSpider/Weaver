...
script_path <- "C:/Users/mcs20/Desktop/EEZA/Weaver/R/pruebas"
...
show_world_info <- function(config_path, resource_species, animal_species, time_step_interval) {
    ...
    for(timeStep in unique(cells_info$timeStep)) {
        fig <- plot_ly()

        fig <- show_grid(fig, grid_size)

        for (species in resource_species) {
            fig <- show_resource_species_info(fig, species, resource_species_info[[species]][timeStep == timeStep, ], colors[[species]])
        }

        figList[[as.character(timeStep)]] <- fig
    }
...
figList <- show_world_info(config_path, resource_species, animal_species, time_step_interval = 1)



ui <- fluidPage(
    titlePanel("Animación de Habitat Shift"),
    sidebarLayout(
        sidebarPanel(
...
server <- function(input, output) {
    output$plot <- renderPlotly({
        return(figList[[as.character(input$timeStep)]])
    })
}
...
# fig <- show_world_info(fig, config_path, resource_species, animal_species, time_step_interval = 1)
