...
const string ValidatorJSON::TypeElemJSON::generateAvailableValues()
{
	...
	std::string values(typeNames[0]);
	for(size_t i = 1; i < typeNames.size(); i++) {
		values += ", " + std::string(typeNames[i]);
	}
...
void ValidatorJSON::addNullProperty(json& properties, const json& nullPropertyToAdd, const string patternElem) 
{
	for(size_t i = 0; i < nullPropertyToAdd.size(); i++) {
		try
		{
			if(!nullPropertyToAdd[i].is_string()) {
				throwValidatorSchemaJSONException("': Not an element of type 'string'");
			}

			if(patternElem != "" && nullPropertyToAdd[i] == PATTERN_ELEM) {
				properties[patternElem]["nullProperty"] = true;
			}
			else {
				properties[nullPropertyToAdd[i]]["nullProperty"] = true;
			}
		}
		catch(ValidatorSchemaJSONException& e)
		{
...
void ValidatorJSON::removeNullProperty(json& properties, const json& nullPropertyToRemove, const string patternElem) 
{
	for(size_t i = 0; i < nullPropertyToRemove.size(); i++) {
		try
		{
			if(!nullPropertyToRemove[i].is_string()) {
				throwValidatorSchemaJSONException("': Not an element of type 'string'");
			}

			if(patternElem != "" && nullPropertyToRemove[i] == PATTERN_ELEM) {
				properties[patternElem]["nullProperty"] = false;
			}
			else {
				properties[nullPropertyToRemove[i]]["nullProperty"] = false;
			}
		}
		catch(ValidatorSchemaJSONException& e)
		{
...
void ValidatorJSON::addSchema(json& schema, const json& newSchema, const json& config, const string patternElem) {
	...
	try
	{
		if(!newSchema.at("nullProperty").is_array()) {
			throwValidatorSchemaJSONException("': Not an element of type 'array'");
		}

		addNullProperty(schema["properties"], newSchema["nullProperty"], patternElem);
	}
	catch(ValidatorSchemaJSONException& e)
	{
...
void ValidatorJSON::removeSchema(json& schema, const json& schemaToRemove, const string patternElem)
{
	...
	try
	{
		if(!schemaToRemove.at("nullProperty").is_array()) {
			throwValidatorSchemaJSONException("': Not an element of type 'array'");
		}

		removeNullProperty(schema["properties"], schemaToRemove["nullProperty"], patternElem);
	}
	catch(ValidatorSchemaJSONException& e)
	{
		e.addPreMessage(".nullProperty");
...
const json ValidatorJSON::solveEqualOperator(const json& config, const json& condition, const string patternElem) {
	...
	if(lhsResult.is_number_float() || rhsResult.is_number_float())
	{
		return fabs(static_cast<double>(lhsResult) - static_cast<double>(rhsResult)) <= numeric_limits<double>::epsilon();
	}
	else
	{
...
const json ValidatorJSON::solveLessEqualThanOperator(const json& config, const json& condition, const string patternElem) {
	...
	if(lhsResult.is_number_float() || rhsResult.is_number_float())
	{
		return static_cast<double>(lhsResult) <= static_cast<double>(rhsResult) + numeric_limits<double>::epsilon();
	}
	else
	{
		return lhsResult <= rhsResult;
...
const json ValidatorJSON::solveOperator(const json& config, const json& condition, const string patternElem) {
	...
	{
		throwValidatorSchemaJSONException("': Property 'type' not defined. Valid values are " + string(OperatorJSON::printAvailableValues()));
	}
	...
	switch (type)
	{
		...
		case OperatorJSON::constant: {
			try
			{
				if(condition.at("value").is_array() || condition.at("value").is_object()) {
					throwValidatorSchemaJSONException("': Not an element of type 'boolean', 'integer', 'unsigned_integer', 'floater', 'string' or 'null'");
				}
		...
		case OperatorJSON::subtract: {
			...
			{
				throwValidatorSchemaJSONException("': Property 'result_type' not defined. Valid values are integer, unsigned_integer, floater");
			}
			...
			try
			{
				switch (TypeElemJSON::stringToEnumValue((string)condition["result_type"])) {
					...
					case TypeElemJSON::floater: {
						return solveSubtractOperator<double>(config, condition, patternElem);
						break;
					}
					default: {
			...
			{
				throwValidatorSchemaJSONException(".result_type': Unknown type '" + string(condition["result_type"]) + "'. Valid values are integer, unsigned_integer, floater");
			}
			break;
		}
		case OperatorJSON::sum: {
			...
			try
			{
				switch (TypeElemJSON::stringToEnumValue((string)condition["result_type"])) {
					...
					case TypeElemJSON::floater: {
						return solveSumOperator<double>(config, condition, patternElem);
						break;
					}
					default: {
						throwValidatorSchemaJSONException(".type': Default case");
		...
		case OperatorJSON::division: {
			...
			{
				throwValidatorSchemaJSONException("': Property 'result_type' not defined. Valid values are integer, floater");
			}
			...
			try
			{
				switch (TypeElemJSON::stringToEnumValue((string)condition["result_type"])) {
					...
					case TypeElemJSON::floater: {
						return solveDivisionOperator<double>(config, condition, patternElem);
						break;
					}
					default: {
						throwValidatorSchemaJSONException(".type': Default case");
			...
			{
				throwValidatorSchemaJSONException(".result_type': Unknown type '" + string(condition["result_type"]) + "'. Valid values are integer, floater");
			}
			break;
		}
		case OperatorJSON::multiply: {
			...
			try
			{
				switch (TypeElemJSON::stringToEnumValue((string)condition["result_type"])) {
					...
					case TypeElemJSON::floater: {
						return solveMultiplyOperator<double>(config, condition, patternElem);
						break;
					}
					default: {
						throwValidatorSchemaJSONException(".type': Default case");
	...
	try
	{
		...
		for(const json &warningCondition : schema["warningConditions"]) {
			if(checkIf(config, warningCondition["condition"])) {
				cout << "[WARNING] " << static_cast<string>(warningCondition["warning"]) << endl;
...
void ValidatorJSON::validateArray(const json& config, json& schema) {
	...
	ItemsOrder::ItemsOrderType itemsOrder = ItemsOrder::increasing;
...
void ValidatorJSON::validateFloat(const json& config, json& schema) {
	try
	{
		try
		{
			if(!schema.at("minimum").is_number_float()) {
				throwValidatorSchemaJSONException("': Not an element of type 'floater'");
			}
		}
		catch(ValidatorSchemaJSONException& e)
		{
		...
		if( (exclusiveMinimum && (double)config <= (double)schema["minimum"]+numeric_limits<double>::epsilon()) || (!exclusiveMinimum && (double)config <= (double)schema["minimum"]-numeric_limits<double>::epsilon()) ) {
			throwValidatorConfigJSONException("': Value less than " + string((exclusiveMinimum) ? "or equal " : "") + "minimum");
		}
	}
	catch(const json::out_of_range& e) {}
	...
	try
	{
		...
		if( (exclusiveMaximum && (double)config >= (double)schema["maximum"]-numeric_limits<double>::epsilon()) || (!exclusiveMaximum && (double)config >= (double)schema["maximum"]+numeric_limits<double>::epsilon()) ) {
			throwValidatorConfigJSONException("': Value greater than " + string((exclusiveMaximum) ? "or equal " : "") + "maximum");
		}
	}
	catch(const json::out_of_range& e) {}
...
void ValidatorJSON::validateElem(const json& config, json& schema) {
	...
	{
		throwValidatorSchemaJSONException("': Property 'type' not defined. Valid values are " + string(ValidatorJSON::TypeElemJSON::printAvailableValues()));
	}
	...
	try
	{
		if(!schema.at("nullProperty").is_boolean()) {
			throwValidatorSchemaJSONException("': Not an element of type 'boolean'");
		}

		nullProperty = schema["nullProperty"];
	}
	catch(ValidatorSchemaJSONException& e)
	{
		e.addPreMessage(".nullProperty");
	...
	if(!nullProperty)
	{
		if(config.is_null())
		{
			throwValidatorSchemaJSONException("': Cannot be null");
		}
		else 
		{
			switch (type)
