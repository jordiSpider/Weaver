...
void Config::moveFolder(vector<string> &&initial_folder_path, vector<string> &&final_folder_path)
{
    ...
    unique_ptr<Folder> folderToMove = move(initial_root_folder->getSubFolder(initial_folder_path.back()));
    final_root_folder->setFolder(final_folder_path.back(), folderToMove);
    initial_root_folder->removeFolder(initial_folder_path.back());
}
