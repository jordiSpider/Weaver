...
{
	if(name != Trait::factorEggMass && name != Trait::pheno)
	{
		if(newValue < 0.0)
		{
			throwLineInfoException("The trait '" + string(Trait::to_string(name)) + "' will have a negative value.");
		}
	}
...
//Dinosaurs (really only?) should by daily target - attempt because current voracities are ridiculous
  ...
  /* if (isMature() && relativeHumidity<100){
	...
	double postTvor = MathFunctions::useJohnsonLewis(
		newT, getTrait(Trait::actE_vor), getSpecies()->getTrait(Trait::voracity)->getLowerThreshold(),
		getSpecies()->getTrait(Trait::voracity)->getUpperThreshold(), ToptVoracity, getSpecies()->getEdVoracity(),
		voracity_ini
	);
 ...
 /*  if(getId() == 1 && timeStep == 40){
	...
	postTsearch = MathFunctions::useJohnsonLewis(
		newT, getTrait(Trait::actE_search), getSpecies()->getTrait(Trait::search_area)->getLowerThreshold(),
		getSpecies()->getTrait(Trait::search_area)->getUpperThreshold(), ToptSearch, getSpecies()->getEdSearch(),
		preTsearch
	);
	...
	postTspeed = MathFunctions::useJohnsonLewis(
		newT, getTrait(Trait::actE_speed), getSpecies()->getTrait(Trait::speed)->getLowerThreshold(),
		getSpecies()->getTrait(Trait::speed)->getUpperThreshold(), ToptSpeed, getSpecies()->getEdSpeed(),
		preTspeed
	);
...
void Animal::calculateGrowthCurves(Temperature temperature, ostream& tuneTraitsFile, bool printGrowthData, double ageAtInitialization) //Dinosaurs
{
	...
	double changedKmin = linearInterpolate(dellsNewT, johnsonLewis(xmin, Topt, getSpecies()->getEdGrowth(), activationEnergyGrowth), johnsonLewis(xmax, Topt, getSpecies()->getEdGrowth(), activationEnergyGrowth), changeKmin, getSpecies()->getMinTraitRange(Trait::growth));
	double changedKmax = linearInterpolate(dellsNewT, johnsonLewis(xmin, Topt, getSpecies()->getEdGrowth(), activationEnergyGrowth), johnsonLewis(xmax, Topt, getSpecies()->getEdGrowth(), activationEnergyGrowth), changeKmin, getSpecies()->getMaxTraitRange(Trait::growth));
*/
...
unique_ptr<list<AnimalInterface*>> Animal::breedAsexually( int objectiveEggsNumber,int timeStep, Temperature temperature, bool saveAnimalConstitutiveTraits, ofstream &constitutiveTraitsFile)
{
	...
	while( createdEggsNumber < objectiveEggsNumber )
	{
		...
		AnimalInterface* newOffspring = createOffspring(femaleGameteSelected, maleGameteSelected, getMutableTerrainCellInterface(), getTrait(Trait::factorEggMass), timeStep, generationNumberFromFemaleParent + 1, generationNumberFromMatedMale + 1, getId(), idFromMatedMale, getSpecies(), offSpringGender, true);
