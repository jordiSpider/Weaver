...
public:
    ...
    SpatialTree(const nlohmann::json &mapConfig, const nlohmann::json &spatialTreeConfig, World* const world);
    virtual ~SpatialTree();
    ...
    void updateMoisture();
    void update(const unsigned int numberOfTimeSteps, std::ofstream &tuneTraitsFile);
    ...
    std::pair<std::unique_ptr<std::vector<InstarVector<std::vector<AnimalStatistical*>>>>, unsigned int> generateStatisticsPopulation(std::vector<InstarVector<std::vector<std::vector<TerrainCellInterface*>::iterator>>> &mapSpeciesInhabitableTerrainCells);
    const unsigned int generatePopulation(AnimalSpecies* currentAnimalSpecies, const InstarVector<std::vector<std::vector<TerrainCellInterface*>::iterator>> &speciesInhabitableTerrainCells);
