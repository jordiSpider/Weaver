...
protected:
    virtual void activateAndResumeAnimals(const unsigned int numberOfTimeSteps)=0;
...
public:
    ...
    virtual Map& getMutableMap() const=0;
    virtual const World* const getWorld() const=0;
    virtual World* const getMutableWorld() const=0;
    ...
    virtual std::tuple<bool, TerrainCellInterface*, TerrainCellInterface*, Animal*, unsigned int> randomInsertAnimal(const Instar &instar, AnimalSpecies* animalSpecies, const bool isStatistical)=0;
    virtual void addAnimal(const LifeStage &lifeStage, const unsigned int animalSpeciesId, const Instar &instar, const AnimalSpecies::Gender::GenderValue &gender, Animal* const newAnimal)=0;
    virtual void eraseAnimal(Animal* const animalToRemove)=0;
    virtual void eraseAnimal(const LifeStage &lifeStage, const unsigned int animalSpeciesId, const Instar &instar, const AnimalSpecies::Gender::GenderValue &gender, Animal* const animalToRemove)=0;
    virtual void eraseAllAnimals()=0;

    virtual void changeAnimalToSenesced(Animal* targetAnimal, const unsigned int numberOfTimeSteps)=0;

    virtual void updateMoisture()=0;
    virtual void update(const unsigned int numberOfTimeSteps, std::ostream& tuneTraitsFile)=0;

    virtual void setCopied(const bool newCopiedValue)=0;

    virtual void migrateAnimalTo(AnimalNonStatistical* animalToMigrate, TerrainCellInterface* newTerrainCell, const PointContinuous &newPosition)=0;

    virtual std::tuple<double, double, double, const EdibleInterface*> getCellEvaluation(AnimalNonStatistical* animalWhoIsEvaluating, std::pair<TerrainCellInterface::TerrainCellCoverage, std::unique_ptr<Ring>> &evaluationArea, const std::list<const EdibleInterface*> &ediblesHasTriedToPredate, double muForPDF, double sigmaForPDF, double predationSpeedRatioAH, double predationHunterVoracityAH, double predationProbabilityDensityFunctionAH, double predationSpeedRatioSAW, double predationHunterVoracitySAW, double predationProbabilityDensityFunctionSAW, double encounterHuntedVoracitySAW, double encounterHunterVoracitySAW, double encounterVoracitiesProductSAW, double encounterHunterSizeSAW, double encounterHuntedSizeSAW, double encounterProbabilityDensityFunctionSAW, double encounterHuntedVoracityAH, double encounterHunterVoracityAH, double encounterVoracitiesProductAH, double encounterHunterSizeAH, double encounterHuntedSizeAH, double encounterProbabilityDensityFunctionAH)=0;
    ...
    virtual void dieFromBackground(const unsigned int numberOfTimeSteps)=0;
    virtual void assimilateFoodMass()=0;
    virtual void metabolizeAnimals()=0;
    virtual void growAnimals()=0;
    virtual std::ostream& printAnimalsVoracities(std::ostream& os)=0;
    ...
    virtual void breedAnimals(const unsigned int numberOfTimeSteps, boost::filesystem::path outputDirectory, bool saveAnimalConstitutiveTraits, std::ofstream &constitutiveTraitsFile)=0;

    virtual double turnEdibleIntoDryMassToBeEaten(EdibleInterface &targetAnimal, const double &targetDryMass, const Ring* const perceptionArea, const unsigned int numberOfTimeSteps, AnimalNonStatistical* predatorEdible, double leftovers)=0;
