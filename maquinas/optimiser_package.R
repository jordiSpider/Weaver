...
WeaverOptimiserClass <- R6Class("WeaverOptimiserClass",
    private = list(
        ...
        set_optimisation_mode = function(minimise) {
            private$minimise = minimise

            if(private$minimise) {
                self$display("Optimisation mode: Minimise")
            }
            else {
                self$display("Optimisation mode: Maximise")
            }
        },
        ...
        create_config = function(individual, output_path, base_config_path) {
            ...
            for(name in names(private$parameters)) {
                if(name == "isoperlaSearchArea") {
                    search_area_ini <- individual[[name]]

                    femaleWetMass = 27.8513822479759
                    ...
                    json_data <- fromJSON(file.path(config_path, private$parameters[[name]]$files[[1]]$file_path), simplifyVector = FALSE)


                    eval(parse(text = "json_data$animal$genetics$traits$definition$search_area$speciesLevelParams$value <- search_area_ini"))
                    eval(parse(text = "json_data$animal$genetics$traits$definition$scope_area$speciesLevelParams$value <- scope_area_ini"))
                    eval(parse(text = "json_data$animal$genetics$traits$definition$interaction_area$speciesLevelParams$value <- interaction_area_ini"))


                    write(toJSON(json_data, auto_unbox = TRUE, pretty = TRUE, null = "null", digits = 17), file.path(config_path, private$parameters[[name]]$files[[1]]$file_path))
                }
                else {
                    if(class(private$parameters[[name]])[1] == "Abundances") {
                        for(i in seq_along(private$parameters[[name]]$files)) {
                            json_data <- fromJSON(file.path(config_path, private$parameters[[name]]$files[[i]]$file_path), simplifyVector = FALSE)
                            ...
                            eval(parse(text = paste0("json_data$", private$parameters[[name]]$files[[i]]$json_path, " <- individualsPerInstar")))

                            write(toJSON(json_data, auto_unbox = TRUE, pretty = TRUE, null = "null", digits = 17), file.path(config_path, private$parameters[[name]]$files[[i]]$file_path))
                        }
                    }
                    else {
                        for(i in seq_along(private$parameters[[name]]$files)) {
                            ...
                            eval(parse(text = paste0("json_data$", private$parameters[[name]]$files[[i]]$json_path, " <- individual[[name]]")))
        ...
        evaluate_population = function(population, fitness_class, fitness_function, program, output_path, base_config_path) {
            ...
            if(nrow(population_to_evaluate) > 0)
            {
                ...
                new_fitness_values <- lapply(result_folder_vector, function(result_folder) fitness_function(result_folder))
            ...
            sapply(1:length(population), function(i) {
                ...
                sapply(fitness_class$get_display_info(fitness_values[[i]]$fitness), function(info) {
        ...
        selection = function(population, fitness_class, fitness_values, k, size) {
            ...
            for (i in seq(1, size)) {
                ...
                best_candidate <- candidates[[fitness_class$get_best_index(private$minimise, fitness_values[candidates])]]
        ...
        next_generation = function(population, fitness_class, fitness_values, keepBest, tournament_size, crossover_prob, mutation_rate) {
            ...
            if(keepBest) {
                best_candidate <- fitness_class$get_best_index(private$minimise, fitness_values)
    ...
    public = list(
        ...
        RealValued = R6Class("RealValued",
            ...
            public = list(
                ...
                initialize = function(name, files, min, max, constraints) {
                    self$name <- name

                    self$files <- files

                    for(i in seq_along(self$files)) {
                        file_path <- strsplit(self$files[[i]]$file_path, "/")[[1]]
                        file_path <- do.call(file.path, as.list(file_path))

                        self$files[[i]]$file_path <- file_path
                    }
                ...
                show = function() {
                    ...
                    for(i in seq_along(self$files)) {
                        WeaverOptimiser$display(paste0("  File path ", i, ": \"", self$files[[i]]$file_path, "\""))
                        WeaverOptimiser$display(paste0("  JSON path ", i, ": \"", self$files[[i]]$json_path, "\""))
                    }
        ...
        Permutation = R6Class("Permutation",
            ...
            public = list(
                ...
                initialize = function(name, files, elements, constraints) {
                    self$name <- name
        ...
        Boolean = R6Class("Boolean",
            ...
            public = list(
                ...
                initialize = function(name, files, constraints) {
                    self$name <- name
        ...
        Abundances = R6Class("Abundances",
            ...
            public = list(
                ...
                initialize = function(name, files, densitiesVector, min, max, constraints) {
                    self$name <- name
        ...
        WeightedFitness = R6Class("WeightedFitness",
            ...
            private = list(
                ...
                calculate_fitness = function(values) {
                    return(sum(mapply(`*`, private$weights, values[names(private$weights)])))
                }
            ),
            public = list(
                ...
                get_row_info = function(values) {
                    return(c(unname(values), private$calculate_fitness(values)))
                },

                display_generation_summary = function(generation, best_fitness, mean_fitness) {
                ...
                get_best_index = function(minimise, values) {
                    if(minimise) {
                        return(Reduce(function(a, b) if (private$compare_min(values[[a]], values[[b]])) a else b, seq_along(values)))
                    }
                    else {
                        return(Reduce(function(a, b) if (private$compare_max(values[[a]], values[[b]])) a else b, seq_along(values)))
                    }
                },
                ...
                calculate_mean_value = function(population) {
                    ...
                    for (name in names(private$weights)) {
                        mean_value[[name]] <- mean(sapply(population, function(values) { values[[name]] }))
                    } 
        ...
        MultiObjectiveFitness = R6Class("MultiObjectiveFitness",
            inherit = WeaverOptimiserClass$Fitness,
            private = list(
                objectives_order = character(0),

                compare_min = function(a, b) {
                    for (objetive in private$objectives_order) {
                        if(a[[objetive]] < b[[objetive]]) {
                            return(TRUE)
                        }
                        else if(a[[objetive]] > b[[objetive]]) {
                            return(FALSE)
                        }
                    }
            ...
            public = list(
                initialize = function(objectives_order) {
                    private$objectives_order <- objectives_order
                },

                get_display_info = function(values) {
                    return(sapply(private$objectives_order, function(name) {
                        paste0("Name: ", name, " | Value: ", values[[name]])
                    }))
                },

                get_header = function() {
                    return(private$objectives_order)
                },

                get_row_info = function(values) {
                    return(unname(values[private$objectives_order]))
                },

                display_generation_summary = function(generation, best_fitness, mean_fitness) {
                    WeaverOptimiser$display("Best fitness:")
                    sapply(private$objectives_order, function(name) {
        ...
        run = function(minimise, population_size, max_generations, fitness_class, fitness_function, program, output_path, base_config_path, keepBest, tournament_size, crossover_prob, mutation_rate, seed, num_cores, debug) {
            ...
            private$set_optimisation_mode(minimise)
            ...
            for (generation in 1:max_generations) {
                ...
                result <- private$evaluate_population(population, fitness_class, fitness_function, program, output_path, base_config_path)
                ...
                best_fitness <- c(best_fitness, list(fitness_values[[fitness_class$get_best_index(private$minimise, fitness_values)]]))

                mean_fitness <- c(mean_fitness, list(fitness_class$calculate_mean_value(fitness_values)))
                
                fitness_class$display_generation_summary(generation, best_fitness[[length(best_fitness)]], mean_fitness[[length(mean_fitness)]])
